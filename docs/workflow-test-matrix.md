...
| Workflows / queue | Server covers trigger filters, bounded execution, retry, and checkpoint/approval semantics; the pipeline editor it used to be driven through is gone, so scheduled syncs and promoted workflows are the only ways a run starts | **Not production-scale:** queue is process-local; no durable cross-instance lease/recovery yet |
