...
class TaskSubjectTests(unittest.TestCase):
    def test_create_task_persists_optional_typed_subject(self) -> None:
        ...
        with patch.object(mutations_knowledge, "exec_", side_effect=lambda sql, args=(): writes.append((sql, args))), \
        ...
        self.assertEqual(writes[0][1], (
            "Review retention claim", "Dana Rodriguez", "DR", "factcheck", "Fact check",
    ...
    def test_tasks_query_exposes_subject_reference(self) -> None:
        ...
        with patch.object(queries, "q", return_value=[row]):
