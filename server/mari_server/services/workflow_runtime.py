...
def _step_fact_check(cfg: dict, ctx: dict) -> tuple[str, str, dict]:
    from mari_server.services.knowledge import fact_check_document
    contradictions = 0
    checked = 0
...
def _step_derive_links(cfg: dict, ctx: dict) -> tuple[str, str, dict]:
    from mari_server.services.knowledge import derive_links
    added = derive_links()
...
def _step_scan_facts(cfg: dict, ctx: dict) -> tuple[str, str, dict]:
    ...
    from mari_server.services.knowledge import scan_facts_for
...
def _step_scan_decisions(cfg: dict, ctx: dict) -> tuple[str, str, dict]:
    ...
    from mari_server.services.knowledge import scan_decisions_for
    doc_ids = ctx.get("doc_ids") or None
