...
const productRoutes = [
  ...
  "/publish", "/publish?tab=mcp", "/publish?tab=bots", "/sources",
...
test("LIVE a promoted workflow is shown on the run that produced it", async ({ page }) => {
  await signIn(page);
  const data = await graphql<{ trajectories: {
    id: number; macroIntent: string;
    promotedWorkflow: { name: string; nodeCount: number } | null;
  }[] }>(page, `{ trajectories(limit: 50) { id macroIntent promotedWorkflow { name nodeCount } } }`);
  const reviewed = data.trajectories.find((row) => row.promotedWorkflow);
  test.skip(!reviewed, "No workflow has been reviewed and promoted in this workspace.");

  /* Promotion used to answer with a button that navigated to /flows. That page
     is gone, and even while it existed, leaving the run behind was the only
     way to find out whether the button had done anything. The workflow it made
     is drawn on the card now, so this asserts the state, not a URL. */
  ...
  await expect(page.getByRole("dialog")).toContainText("Chronological evidence");
});

test("LIVE the retired Trajectories and Answers routes land on Workflows", async ({ page }) => {
  await signIn(page);
  await page.goto("/trajectories");
  await expect(page).toHaveURL(/\/workflows$/);
  // The query string is what makes the old links worth honouring: /answers?answer=4
  // is what a Review item and a served chat citation point at.
  await page.goto("/answers?answer=4");
  await expect(page).toHaveURL(/\/workflows\?answer=4&tab=answers$/);
});
