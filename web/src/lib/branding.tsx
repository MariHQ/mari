...
/** Ink to sit ON the accent: light cream on dark accents, dark ink on light. */
export const inkFor = (hex: string) => (luminance(hex) > 0.42 ? "#2d2a22" : "#f5ead4");
...
export function computeCssVars(b: Branding): Record<string, string> {
  ...
  if (b.displayFont && b.displayFont.trim()) {
    vars["--display"] = withFontFallback(b.displayFont, '"Playfair Display", Georgia, serif');
  }
  // bodyFont maps to --serif: body prose is set in var(--serif) (Lora), so
  // that is "the body family" here. --sans (labels/meta) stays Mari's.
  if (b.bodyFont && b.bodyFont.trim()) {
    vars["--serif"] = withFontFallback(b.bodyFont, "Georgia, serif");
  }
  return vars;
}
