    ...
    <div className="lineage-toolbar">
      <div className="control-group control-group--wrap">
              ...
              <MenuRadioItem key={l.key} value={l.key}>
                <span className="row lg-mrow">
                  <svg width="22" height="10">
                    {l.key === "source" && <line x1="0" y1="5" x2="22" y2="5" stroke="#35549d" strokeWidth="2" />}
                    {l.key === "stale" && <>{["#43663c", "#c8973a", "#c0392b"].map((c, i) => <circle key={c} cx={4 + i * 7} cy="5" r="3" fill={c} />)}</>}
                    {l.key === "owner" && <>{OWNER_PALETTE.slice(0, 3).map((c, i) => <circle key={c} cx={4 + i * 7} cy="5" r="3" fill={c} />)}</>}
                    {l.key === "health" && <circle cx="8" cy="5" r="4" fill="#c0392b" />}
                  </svg>
                  {l.label}
                </span>
              </MenuRadioItem>
        ...
        <Menu
          ...
          trigger={(
            <Button compact>
              Layout <b className="lg-semibold">{layout === "flow" ? "Flow" : "Timeline"}</b> <Ic.Chev size={13} />
            </Button>
          )}
        >
          <MenuRadioGroup value={layout} onValueChange={(v) => onLayout(v as LayoutMode)}>
            {([["flow", "Flow (left → right)"], ["timeline", "Timeline (x = time)"]] as [LayoutMode, string][]).map(([k, label]) => (
