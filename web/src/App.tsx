...
import { CloudLogo, Pulseline, SketchDefs, Sprig } from "./components/art";
...
import sidebarBotanical from "./assets/illustrations/sidebar-botanical.png";
...
const NAV = [
  { to: "/", label: "Overview", icon: Ic.Home },
  { to: "/knowledge", label: "Knowledge", icon: Ic.Book },
  { to: "/answers", label: "Answers", icon: Ic.CheckCircle },
  { to: "/decisions", label: "Decisions", icon: Ic.Quill },
  { to: "/library", label: "Library", icon: Ic.Tag },
  { to: "/lineage", label: "Lineage", icon: Ic.LineageIcon },
  { to: "/facts", label: "Facts", icon: Ic.ShieldCheck },
  { to: "/flows", label: "Flows", icon: Ic.Flow },
  { to: "/publish", label: "Publish", icon: Ic.Send },
  { to: "/insights", label: "Insights", icon: Ic.Sparkle },
];
...
function Sidebar({ railed, onToggleRail }: { railed: boolean; onToggleRail: () => void }) {
  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="nav">
        {NAV.map((n) => (
          <NavLink key={n.to} to={n.to} end={n.to === "/"} className={({ isActive }) => `nav__item${isActive ? " active" : ""}`} title={n.label}>
            <n.icon size={19} />
      ...
      <div className="sidebar__art">
        <img src={sidebarBotanical} alt="" aria-hidden="true" />
      </div>

      <button className="sidebar__help" onClick={() => toggleDock(true)}>
        <Sprig size={16} color="#7d8a5c" />
        <span>
          <b>Need help?</b>
