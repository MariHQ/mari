// Hand-drawn art: hatched swatches, botanical line work, pencil mountains,
// woven-cloud logo. Everything is inline SVG so the whole aesthetic ships
...
export function CloudLogo({ size = 84 }: { size?: number }) {
  ...
  return (
    <svg width={size} height={size * 0.72} viewBox="0 0 100 72" fill="none" aria-hidden>
      ...
      <g clipPath="url(#cloudclip)" filter="url(#cloud-wobble)" stroke="#f7ecd7" strokeLinecap="round">
      ...
      <path d={CLOUD_PATH} stroke="#f7ecd7" strokeWidth="3.1" strokeLinecap="round" strokeLinejoin="round" filter="url(#cloud-wobble)" />
      <path d="M22 62 C38 60 57 64 76 61" stroke="#8b351f" strokeWidth="1.2" opacity=".38" strokeLinecap="round" />
    </svg>
  );
}

// teardrop leaf: base at (x,y), pointing along angle deg, length l
function leaf(x: number, y: number, deg: number, l: number) {
  const w = l * 0.42;
  return (
    <path
      d={`M 0 0 Q ${l * 0.45} ${-w} ${l} 0 Q ${l * 0.45} ${w} 0 0 Z`}
      transform={`translate(${x} ${y}) rotate(${deg})`}
    />
  );
}

export function Sprig({ size = 60, color = "#6d7a55", flip = false }: { size?: number; color?: string; flip?: boolean }) {
  return (
    <svg
      width={size}
      height={size * 1.9}
    ...
    >
      <g stroke={color} strokeWidth="1.3" strokeLinecap="round">
        <path d="M20 74 C 18 52 16 30 22 4" fill="none" strokeWidth="1.6" />
        {[...Array(6)].map((_, i) => {
          const y = 13 + i * 10;
          const l = 13 - i * 0.8;
          return (
            <g key={i}>
              {leaf(19.5 - i * 0.4, y, -142, l)}
              {leaf(20.5 - i * 0.4, y + 4, -38, l)}
            </g>
          );
        })}
      </g>
...
export function FadedSprigLarge({ color = "#aca387" }: { color?: string }) {
  return (
    <svg width="120" height="80" viewBox="0 0 120 80" fill="none" aria-hidden>
      <g stroke={color} strokeWidth="1.2" strokeLinecap="round" opacity="0.75">
        <path d="M8 76 C 40 62 78 42 112 12" fill="none" />
        {[...Array(5)].map((_, i) => {
          const t = i / 4;
          const x = 20 + t * 72;
          const y = 68 - t * 42;
...
export function Bars({ values, color = "#5c7a4c", width = 66, height = 24 }: { values: number[]; color?: string; width?: number; height?: number }) {
...
export function Pulseline({ color = "#6b6353" }: { color?: string }) {
  return (
...
const TEX_TONES = ["#a8724f", "#7e8a63", "#6b7d99", "#b08a4a", "#96604a"];
