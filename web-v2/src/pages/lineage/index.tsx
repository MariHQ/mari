...
export default function LineagePage() {
    ...
    for (const n of nodes) {
      ...
      ns.push({
        ...
        data: {
          ...
          ring: n.warn ? "#b23a1e" : "#d6dbe1",
          uc: SOURCE_ACCENT[srcKey(n.source)] ?? "#6f7c89",
        },
      ...
      if (stub) {
        ns.push({
          group: "nodes" as const,
          data: { id: stub.id, label: `+${stub.rest} more`, ring: "#d6dbe1", uc: SOURCE_ACCENT.github },
  ...
  useEffect(() => {
    ...
    if (layout === "circle" && cy.nodes().length > 0) {
      // every file on one ring, ordered so connected files sit near each
      // other — edges cut across the circle and read as a relationship web.
      // avoidOverlap is off on purpose: with 500+ variable-width labels it
      // forces a radius so large that even minZoom can't fit it. Instead we
      // size the ring ourselves (~6px of circumference per node, capped so
      // the whole circle always stays within minZoom's fittable range).
      const n = cy.nodes().length;
      const radius = Math.min(1800, Math.max(280, n * 6));
      cy.layout({
        name: "circle",
        fit: true,
        padding: 60,
        avoidOverlap: false,
        radius,
        sort: (a: any, b: any) => b.degree(false) - a.degree(false),
        animate: false,
      } as any).run();
    }
    if (layout === "cluster" && cy.nodes().length > 0) {
      // force-directed: docs that reference/derive/discuss each other pull
      // together into tight blast-radius clusters; componentSpacing pries
      // unrelated clusters apart so the separation itself is the signal.
      // Same orphan-splitting trick as flow — a 0-degree node has no cluster
      // to join, and letting cose fight over its position wastes iterations.
      const orphans = cy.nodes().filter((n) => n.degree(false) === 0);
      const linked = cy.nodes().not(orphans);
      if (linked.length > 0) {
        linked.union(linked.edgesWith(linked))
  ...
  useEffect(() => {
    ...
    cy.batch(() => {
      cy.nodes().forEach((cn) => {
        ...
        let ring = n.warn ? "#b23a1e" : "#d6dbe1";
        let uc = SOURCE_ACCENT[srcKey(n.source)] ?? "#6f7c89";
        switch (lens) {
          case "stale": {
    ...
    <div className="lg-page">
      ...
      <div className="card lg-card">
        ...
        <div className="lg-legend">
          ...
          {lens === "stale" && ([["≤ 14 days", "#2c6e49"], ["≤ 45 days", "#a05e1c"], ["> 45 days", "#b23a1e"]] as const).map(([label, c]) => (
          ...
          {lens === "owner" && (
            <>
              ...
              <span className="row" style={{ gap: 6 }}>
                <span className="lg-dot" style={{ background: "#a05e1c" }} /> Unowned
              </span>
            </>
          )}
          {lens === "health" && ([["Healthy", "#2c6e49"], ["Warn / orphan / contradicted / lag", "#b23a1e"]] as const).map(([label, c]) => (
            <span key={label} className="row" style={{ gap: 6 }}>
              <span className="lg-dot" style={{ background: c }} /> {label}
            </span>
          ))}
